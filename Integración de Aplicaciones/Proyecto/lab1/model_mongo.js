        ...
        try{
            ...
            col.findOne({ email: email, password: password }).then(_user => {      
                ...
                if (!_user) {
                    ...
                    logger((messages.log.user_join.replace("%nick%", _user.nick).replace("%email%", _user.email)), 1);
            ...
            cb(printErr(messages.cmd.err.unknown, 0));
        }  
    }).catch(err => {    
        cb(err);  
    }); 
...
function addUser(user, cb) {  
    ...
    if(!user.name || !user.surname || !user.email || !user.nick || !user.password){
        cb(printErr(messages.cmd.addUser.no_param, 0)); return;
    }
        ...
        MongoClient.connect(url).then((client) => {           
            ...
            users.findOne({$or:[{ email: user.email },{ nick: user.nick }] }).then((_user) => { 
                    ...
                    users.insertOne(user).then(result => {              
                        ...
                        /* Mensaje de Log al servidor con la creación del usuario */ 
...
function updateUser(token, user, cb){
    ...
    if(!booleanUser.name && !booleanUser.surname && !booleanUser.email && !booleanUser.password && !booleanUser.nick){
        return cb(printErr(messages.cmd.updateUser.no_param, 0)); 
    }
    MongoClient.connect(url).then(async (client) => {        
        ...
        if(booleanUser.email){ /* Hay que realizar una búsqueda en la base de datos por si coincide con algún email de usuario ya registrado. */
            const existsEmail = await users.findOne({ email: user.email });   
            if(existsEmail) return _cb(printErr(messages.cmd.updateUser.email_exists,0));      
        }
        ...
        if(booleanUser.nick){ /* Hay que realizar una búsqueda en la base de datos por si coincide con algún nick de usuario ya registrado. */
            const existsNick = await users.findOne({ nick: user.nick });               
            if(existsNick) return _cb(printErr(messages.cmd.updateUser.nick_exists,0));                  
        }
        ...
        users.findOne(consulta).then((_user) => { 
            ...
            users.updateOne(consulta, update).then(result => { 
                if(result){
                    logger(messages.log.new_update.replace("%name%",_user.name).replace("%surname%",_user.surname)
                        .replace("%email%",_user.email).replace("%nick%",_user.nick).replace("%password%",_user.password));            
                    _cb(null, _user);
                }else{
                    _cb(err) 
                }                                           
        ...
        users.findOne({ _id: new mongodb.ObjectId(token) }).then(_user => {      
            ...
            let jsonQuery = {}; /* Variable para almacenar la query */
                if(opts.q && typeof opts.q === 'string' && opts.q.trim() !== ''){
                    ...
                    catch(err){ return _cb(printErr(messages.cmd.listUsers.invalid_format, 0)); } // Mensaje de inválid format JSON.              
                 }  
                 ...
                 users.find(_query, _opts).toArray().then(_results => {
                    if(_results.length == 0) return _cb(printErr(messages.cmd.listUsers.no_results, 2));
    ...
    MongoClient.connect(url).then(client => {  
        ...
        .then(_user => {      
            ...
            tweets.insertOne(tweet).then(result => {              
                ...
                logger(messages.log.new_tweet.replace("%content%", contentMsg).replace("%nick%", _user.nick));            
            }).catch(err => {              
                _cb(err)            
            });   
        }).catch(err => {      
...
/* Imprime un mensaje con colores de Log */
function logger(message){
    console.log('\x1b[90m%s\x1b[0m','[LOG] \x1b[3m'+message+'\x1b[0m');
}

/* Imprime un mensaje de Error con colores */
function printErr(message, color){
    switch(color){
        case 0: return new Error('\x1b[31m[Error]\x1b[0m ' + message);
        case 1: return new Error('\x1b[32m[Error]\x1b[0m ' + message);
        case 2: return new Error('\x1b[34m[Error]\x1b[0m ' + message);
    }
}
